...
{
    [self requestHeaderUrl:sz_RequestHeader appending:appending httpMethod:methodType withParameters:parameters success:success faile:faile];
}

+(void)requestHeaderUrl:(NSString *)header appending:(NSString *)appending httpMethod:(SZRequestMethodType)methodType withParameters:(NSDictionary *)parameters success:(void (^)(NSDictionary *))success faile:(void (^)(NSError *))faile{
    [self sharedInstance];
    ...
    NSLog(@"url:%@",url);
    if (methodType == SZRequestMethodTypeGet) {
        [service.afnManager GET:url parameters:confirmDic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            if (isSuccess) {
                ...
                if (faile) {
                    NSError *error = [NSError errorWithDomain:NSURLErrorDomain code:[dic[@"errorCode"] integerValue] userInfo:@{NSLocalizedDescriptionKey:dic[@"msg"]}];
...
+(NSDictionary *)confirmParam:(NSDictionary*)paramet
{
    ...
    NSString *HmacMD5 = [self hmac_MD5:firstStr withKey: sz_secret];
    NSLog(@"%@",HmacMD5);
...
{
    NSDictionary *dic = @{@"access_key": sz_access_key,
                          @"appId" : [self bunldID],
                          @"format" : @"json",
